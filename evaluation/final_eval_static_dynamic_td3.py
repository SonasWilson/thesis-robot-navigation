...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Final TD3 evaluation: static_2 vs dynamic_2 environments"
    )
    ...
    parser.add_argument("--out_dir", type=str, default="./final_eval_static_dynamic_td3")

    # Keep these aligned with defaults used in static_2_td3.py and dynamic_2_td3.py
    parser.add_argument(
        "--static_model_template",
        type=str,
        default="td3_static2_navigation_seed{seed}.zip",
    )
    parser.add_argument(
        "--static_vecnorm_template",
        type=str,
        default="vecnormalize_static2_td3_seed{seed}.pkl",
    )
    parser.add_argument(
        "--dynamic_model_template",
        type=str,
        default="td3_dynamic2_navigation_seed{seed}.zip",
    )
    parser.add_argument(
        "--dynamic_vecnorm_template",
        type=str,
        default="vecnormalize_dynamic2_td3_seed{seed}.pkl",
    )
...
def plot_comparison(summary_df: pd.DataFrame, episodes_df: pd.DataFrame, out_dir: str):
    ...
    labels = ["Static2-TD3", "Dynamic2-TD3"]

    fig, axes = plt.subplots(2, 2, figsize=(14, 10), constrained_layout=True)
    fig.suptitle("TD3 Final Evaluation: Static2 vs Dynamic2", fontsize=16, fontweight="bold")
    ...
    fig.savefig(os.path.join(out_dir, "td3_static_dynamic_metrics.png"), dpi=300, bbox_inches="tight")
    fig.savefig(os.path.join(out_dir, "td3_static_dynamic_metrics.pdf"), bbox_inches="tight")
    plt.close(fig)
    ...
    ax2.grid(alpha=0.3)
    fig2.savefig(os.path.join(out_dir, "td3_static_dynamic_reward_distribution.png"), dpi=300, bbox_inches="tight")
    fig2.savefig(os.path.join(out_dir, "td3_static_dynamic_reward_distribution.pdf"), bbox_inches="tight")
    plt.close(fig2)
    ...
    ax3.grid(alpha=0.3)
    fig3.savefig(os.path.join(out_dir, "td3_static_dynamic_cumulative_success.png"), dpi=300, bbox_inches="tight")
    fig3.savefig(os.path.join(out_dir, "td3_static_dynamic_cumulative_success.pdf"), bbox_inches="tight")
    plt.close(fig3)
...
def main():
    ...
    static_summary, static_episodes = evaluate_td3(
        env_type="static2",
    ...
    dynamic_summary, dynamic_episodes = evaluate_td3(
        env_type="dynamic2",
    ...
    summary_csv = os.path.join(args.out_dir, "td3_static_dynamic_summary.csv")
    episodes_csv = os.path.join(args.out_dir, "td3_static_dynamic_episode_metrics.csv")
    summary_json = os.path.join(args.out_dir, "td3_static_dynamic_summary.json")
    ...
    print("=== TD3 Final Evaluation: Static2 vs Dynamic2 ===")
